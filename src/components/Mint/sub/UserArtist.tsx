...
import { getUserArtists } from "@/services/services";
import { PaymentSplitType } from "@/types";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
import { BaseDialog } from "@/components/Modules/BaseDialog";

export default function UserArtist() {
  const activeAccount = useActiveAccount();
  const [open, setOpen] = useState(false)
  const { userArtists, setUserArtists, selectedArtist, setSelectedArtist, basicDetail, advancedDetails, setBasicDetail, setPaymentSplits, setAdvancedDetails, advancedOptions, setAdvancedOptions, curation } = useCreateNFT();
  ...
  useEffect(() => {
    if (selectedArtist) {
      ...
      if (userArtist) {
        ...
        paymentSplits.push({
          paymentWallet: curation?.owner?.wallet,
  ...
  return (
    ...
    >
      ...
      <PopoverTrigger asChild>
        <Button
          role="combobox"
          className="w-full bg-dark justify-between !h-13 text-white-80"
        >
